...
def help_text() -> str:
    return (
        ...
        "  recovery-email add --email <address> [--password-env <name>]\n"
        "      [--verification auto|proton|interactive]\n"
        "      [--recovery-password-env <name>] [--recovery-proxy <socks5-url>]\n"
        "      [--timeout-seconds <n>]\n"
        "  browser status\n"
        "  browser close [--force]\n"
        "\n"
        "Global options:\n"
        "  --chrome-data-dir <path>  Use an existing Chrome data dir, or create a fresh one inside an empty dir.\n"
